...
file_to_load = "PyParagraph_data/paragraph_2.txt"
file_to_output = "PyParagraph_analysis/paragraph_analysis.txt"
...
with open(file_to_load) as txt_data:
    ...
    #Split the paragraph based on spaces to caluculate word count
...
#round(sum(revenue_change_list)/len(revenue_change_list),2)

            #round(sum(revenue_change_list)/len(revenue_change_list),2)

            #Resplit the original paragraph based on punctuation(.?!)
        ...
        output = (
            ...
            f"Aproximate Sentence Count: {sentence_count}\n"
